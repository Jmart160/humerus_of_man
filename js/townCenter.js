...
townCenter.prototype = {
    create: function(){
        ...
        pudding = game.add.sprite(((game.world.width/2) + 240), 180, 'atlas', 'pudding');
        pudding.anchor.setTo(0.5, 0.5);
        pudding.scale.setTo(.2, .2);
        pudding.inputEnabled = true;

        doggo = game.add.sprite(((game.world.width/2)), 350, 'atlas', 'doggo');
        doggo.anchor.setTo(0.5, 0.5);
        doggo.scale.setTo(.4, .4);
        doggo.inputEnabled = true;

        mynth = game.add.sprite(((game.world.width/2) - 320),((game.world.height/2) + 210), 'atlas', 'mynth');
        mynth.anchor.setTo(0.5, 0.5);
        mynth.scale.setTo(-.2, .2);
        mynth.inputEnabled = true;

        barkeep = game.add.sprite(((game.world.width/2) + 270),((game.world.height/2) + 230), 'atlas', 'barkeep');
        barkeep.anchor.setTo(0.5, 0.5);
        barkeep.scale.setTo(-.2, .2);
        barkeep.inputEnabled = true;

        direct = game.add.sprite(((game.world.width/2) + 50), game.world.height - 60, 'atlas', 'arrow');
